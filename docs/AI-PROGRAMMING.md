...
| 模式 | 产出 | 适用 |
|------|------|------|
| **简单编程**（默认） | 纯 C# 单文件（main.cs） | 快速功能、逻辑为主、界面简单 |
| **完全编程** | XAML + 代码后置（.xaml + .xaml.cs 两文件） | 界面复杂、需要精细布局/样式 |

- 简单编程：AI 生成 main.cs（全 C# 构建 UI），放入 seabed/<分类>/<名字>/ 生效；
- 完全编程：AI 生成 <名字>.xaml + <名字>.xaml.cs（partial class + IWidget），放入同目录生效；
- 两种形态系统都能自动编译挂载（检测到 .xaml + .xaml.cs 走 XAML 编译，否则走纯代码编译）。
