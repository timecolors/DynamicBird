...
- **AI 对话（可选）**：仅当你启用 AI 助手并填写服务商信息后，对话内容（含拖入的图片/文件文本）会由本机**直接发送到你选择的大模型服务商**（如 DeepSeek、OpenAI、本地 Ollama 等），不经过灵动鸟的任何服务器。发送的数据受该服务商隐私政策约束；你可在设置中随时关闭 AI 或删除本地对话历史。
