# 灵动鸟「鸟笼」一级分类代码化规范（BIRDCAGE-SPEC）

> 面向：AI 与用户共同编写灵动鸟的**新功能代码**。所有一级分类（小组件/面板功能/面板设计/动画/外观/交互/状态栏）
...
数据目录（`%LOCALAPPDATA%/DynamicBird/birdcage/`）下的每个**一级分类文件夹**对应一类可编程功能：
...
**目录**：`birdcage/状态栏/<名字>/main.cs`（manifest `kind` = `"StatusProvider"`）

**接口**：`DynamicBird.UI.Status.IStatusProvider`
...
public interface IStatusProvider
{
    ...
    string IconText { get; }    // 图标（emoji 或文本，如 "🌡️"）
...
**目录**：`birdcage/动画/<名字>/main.cs`（manifest `kind` = `"Animation"`）

**接口**：`DynamicBird.Animation.IAnimation`
...
鸟笼「复制 AI 提示词」按节点 `Kind` 分发生成不同模板：

| 节点 Kind | 生成模板 | 产出文件去向 |
|-----------|----------|--------------|
| `StatusProvider` | IStatusProvider 完整 .cs 模板 | `birdcage/状态栏/<名字>/main.cs` |
| `Animation`     | IAnimation 完整 .cs 模板     | `birdcage/动画/<名字>/main.cs`   |
| `Widget`/`Panel` | IWidget 面板源码模板（见 WIDGET-SPEC） | `birdcage/小组件|面板功能/<名字>/main.cs` |
