...
{
    /// <summary>
    /// 鸟笼页：左侧配置树（面板设计/动画/外观/交互/状态栏 → 二级/三级），
    /// 右侧以 JSON 编辑选中节点的配置片段；可应用/恢复，后续支持 AI 提示词一键复制。
    /// </summary>
    public partial class BirdcagePage : UserControl
    {
...
";
        ...
        public BirdcagePage(ISettingsService settings)
        {
            InitializeComponent();
            _settings = settings;
            LoadTree();
        ...
        internal DynamicBird.Core.Models.CustomPanelDefinition? CurrentSelectedCustom()
        {
        ...
        private void BtnDeleteNode_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (IsArmed(customId))
            {
                ...
                {
                    DynamicBird.Core.Infrastructure.Logging.LogManager.Error("删除单预设失败", ex);
        ...
        private void CreateSiblingAtLevel(string parentKey, int level)
        {
            try
            {
                ...
                var dlg = new InputDialog("鸟笼 · 创建同级新功能", $"新建{levelName}，输入名称：", defaultName);
                ...
                var list = _settings.CustomPanels;
                list.Add(new DynamicBird.Core.Models.CustomPanelDefinition
                {
            ...
            {
                DynamicBird.Core.Infrastructure.Logging.LogManager.Error("创建同级功能失败", ex);
                txtJsonStatus.Text = "❌ " + ex.Message;
            }
        }
        ...
        private string ExtractJson(ConfigNode node)
        {
            try
            {
                ...
                if (DynamicBird.UI.Birdcage.BuiltinFeatureSources.Sources.TryGetValue(node.Key, out var tpl))
                {
                    return tpl;
                }
        ...
        /// 完整可执行：编译后（ConfigCode.Apply）可应用写回，鸟笼里"保存当前预设→应用"即生效。</summary>
        ...
        {
            string docText = DynamicBird.UI.Birdcage.SettingsFieldDocs.DocOrName(field);
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(_selected.CustomId))
                {
                    ...
                    if (cp != null)
                    {
                        ...
                        if (!cp.TrustedSource)
                        {
                            string sandboxErr = DynamicBird.UI.Widgets.Dynamic.WidgetCompiler.SandboxErrors(src);
                        ...
                        string err = DynamicBird.UI.Widgets.Dynamic.WidgetCompiler.Validate("panel_" + cp.Id, src);
                        if (err.Length > 0)
                        {
                            _errorCustomId = cp.Id;
                            LoadTree();
        ...
        private static string ValidateConfigCode(string userCode)
        {
            try
            {
                string wrapped = "using DynamicBird.Core.Services.Configuration;" + System.Environment.NewLine + userCode;
                return DynamicBird.UI.Widgets.Dynamic.WidgetCompiler.Validate("config_" + Guid.NewGuid().ToString("N").Substring(0, 8), wrapped);
            }
            catch (Exception ex)
            {
        ...
        private void BtnRestore_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var defaults = new DynamicBird.Core.Services.Configuration.SettingsData();
        ...
        {
            var dlg = new InputDialog("鸟笼 · 保存整套预设", "整套预设保存全部配置，输入预设名称：", "整套预设");
            dlg.Owner = System.Windows.Window.GetWindow(this);
        ...
        private void BtnSavePartial_Click(object sender, RoutedEventArgs e)
        {
            ...
            bool isFeature = DynamicBird.UI.Birdcage.BuiltinFeatureSources.Sources.ContainsKey(_selected.Key);
            bool isPanel = DynamicBird.UI.Birdcage.BuiltinFeatureSources.PanelKeys.Contains(_selected.Key);
        ...
        private void BtnApplyPreset_Click(object sender, RoutedEventArgs e)
        {
            ...
            // ★ 记录冲突来源：预设覆盖了哪些内置节点 → 鸟笼左侧高亮、设置页对应分组变灰
            ...
            LoadTree();
            RefreshOwnerSettingsDimming();
            txtJsonStatus.Text = $"✅ 预设已应用：{name}（冲突的内置设置已置灰，鸟笼左侧对应项高亮）";
            if (_selected != null) txtJsonEditor.Text = ExtractJson(_selected);
        }
        ...
        private void ApplyCustomPreset(ConfigNode node)
        {
            ...
            if (!cp.TrustedSource)
            {
                string sandboxErr = DynamicBird.UI.Widgets.Dynamic.WidgetCompiler.SandboxErrors(code);
                if (sandboxErr.Length > 0)
                {
                    txtJsonStatus.Text = "❌ 沙箱拦截：" + sandboxErr;
                    return;
            ...
            var apply = DynamicBird.UI.Widgets.Dynamic.WidgetCompiler.CompileConfigApply(code, out string err);
        ...
        private void RefreshOwnerSettingsDimming()
        {
            try
            {
                if (System.Windows.Window.GetWindow(this) is DynamicBird.UI.Settings.SettingsWindow sw)
                {
        ...
        /// <summary>其他鸟笼：共享平台（导出 .dbp 包 / 导入并提示风险权限）。</summary>
        private void BtnOtherBirdcage_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                var win = new DynamicBird.UI.Birdcage.BirdcageMarketWindow(this)
                {
