...
namespace DynamicBird.UI.Birdcage
{
    ...
    /// token 用 DPAPI 加密存本地（%LOCALAPPDATA%\DynamicBird\github_token.dat）。
    /// </summary>
    public static class GitHubMarketService
    {
        ...
        private const string Repo = "timecolors/DynamicBird";
        ...
        private static string LogFile => Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "DynamicBird", "github_login.log");
        ...
        private static string TokenFile => Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "DynamicBird", "github_token.dat");
        ...
        private static async Task<string?> GetUserAsync(string token)
        {
            try
            {
                ...
                req.Headers.UserAgent.ParseAdd("DynamicBird");
        ...
        private static string RemovePackageFromIndex(string indexJson, string id)
        {
            ...
            var outRoot = new Dictionary<string, object?>
            {
                ...
                ["marketBase"] = "https://cdn.jsdelivr.net/gh/timecolors/DynamicBird@master/market",
                ["packages"] = packages
            };
            ...
            List<string> permissions, string source)
        {
            ...
            catch (Exception ex) { return "放飞失败：" + ex.Message; }
        }
        ...
        public static async Task<string?> UploadConfigAsync(string user, string json)
        {
            ...
            try
            {
                ...
                await PutFileAsync(path, json, sha, "灵动鸟设置云同步（上传）");
                return null;
            }
