...
namespace DynamicBird.UI.Birdcage
{
    ...
    public sealed class PublishWindow : Window
    {
            ...
            string parentKey, string sourceKey, string defaultCategory, System.Collections.Generic.List<string> permissions)
        {
            ...
            root.Children.Add(new TextBlock
            {
                Text = "把当前选中的「" + _defaultName + "」发布到市场，其他灵动鸟用户即可领养。代码将公开托管在 GitHub。",
                FontSize = 11.5,
        ...
        private async Task PublishAsync()
        {
            ...
            try
            {
                ...
                Published = true;
                NameResult = name;
                _status.Text = "✅ 已放飞「" + name + "」！稍后其他灵动鸟用户可在市场领养（CDN 缓存约几分钟）";
