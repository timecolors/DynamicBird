...
$h = Get-PanelHwnd
if ($h -eq [IntPtr]::Zero) { 'PANEL NOT FOUND'; exit 1 }
$r = Get-Rect $h
if ($r) { "SUMMON-MID rect=($($r.Left),$($r.Top))-($($r.Right),$($r.Bottom)) W=$($r.Right-$r.Left) H=$($r.Bottom-$r.Top)" } else { 'SUMMON RECT NULL' }
# 平滑移到顶边左侧
for ($x = 1280; $x -ge 500; $x -= 60) { [Native]::SetCursorPos($x, 3) | Out-Null; Start-Sleep -Milliseconds 15 }
Start-Sleep -Milliseconds 800
$r = Get-Rect $h
if ($r) { "AFTER-LEFT rect=($($r.Left),$($r.Top))-($($r.Right),$($r.Bottom)) W=$($r.Right-$r.Left) H=$($r.Bottom-$r.Top)" } else { 'AFTER-LEFT RECT NULL' }
# 快速来回扫
for ($round = 1; $round -le 3; $round++) {
    ...
    $r = Get-Rect $h
    if ($r) { "round $round T=$($r.Top) L=$($r.Left) W=$($r.Right-$r.Left) H=$($r.Bottom-$r.Top)" } else { "round $round RECT NULL" }
}
Start-Sleep -Milliseconds 1500
$r = Get-Rect $h
if ($r) { "SETTLED rect=($($r.Left),$($r.Top))-($($r.Right),$($r.Bottom)) W=$($r.Right-$r.Left) H=$($r.Bottom-$r.Top)" } else { 'SETTLED RECT NULL' }